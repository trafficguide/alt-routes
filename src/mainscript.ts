...
function prepareDatabaseStats(statsDisplayElm: HTMLElement) {
  ...
  string += " 項路線資料。";
  string +=
    " (<a class='black-text link' href='javascript: displayDetailedStats();'>路線分類統計</a>)";
...
function conductSearch(
  ...
  if (results_DirectCount > 0) {
    directLinesFeedbackElm.innerHTML =
      "有 " + results_DirectCount + " 條直達路線。";
    if (results_InterchangeCount > 0) {
      interchangeLinesFeedbackElm.innerHTML =
        "同時展示 " + results_InterchangeCount + " 組可考慮的轉車組合。";
    } else {
    ...
    if (results_InterchangeCount > 0) {
      interchangeLinesFeedbackElm.innerHTML =
        "有 " + results_InterchangeCount + " 組轉車組合。";
    } else {
      interchangeLinesFeedbackElm.innerHTML =
        "沒有可考慮的轉車組合。<b>請考慮使用其他交通工具</b>，包括但不限於的士。";
      return;
    }
  }
  ...
  for (let i = 0; i < pathfindingResults.length; i++) {
    ...
    let effectStrings = new Array<String>();
    for (let j = 0; j < connections.length - 1; j++) {
      let L1 = connections[j].getLine();
      let L2 = connections[j + 1].getLine();
      let interchange = getInterchangeRuleForPair(L1, L2);
      if (interchange != null) {
        let tempString = "";
    ...
    if (effectStrings.length == 0) {
      ...
      for (let j = 0; j < effectStrings.length; j++) {
